...
export function createTreeNode(value: JSONValue, key: string, level: number = 0, path: (string | number)[] = []): HTMLElement {
...
function startEditingKey(keySpan: HTMLElement, currentKey: string, path: (string | number)[]): void {
  const input = document.createElement("input");
  input.type = "text";
